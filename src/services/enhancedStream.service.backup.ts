...
export class EnhancedStreamService {
  ...
  async createStream(
    ...
    try {
      ...
      this.websocketService.notifyStreamCreated(
        stream.id,
        payer.walletAddress,
  ...
  async withdrawFromStream(
    ...
    try {
      ...
      this.withdrawalService.validateWithdrawal(stream.recipient.walletAddress, 2);

      // Calculate claimable amount using off-chain simulation
      const claimableAmount = this.balanceService.calculateClaimableAmount(stream);
      ...
      this.websocketService.notifyStreamCancelled(
        streamId,
  ...
  async getClaimableAmount(streamId: string): Promise<string> {
    try {
      ...
      return this.balanceService.calculateClaimableAmount(stream);
    } catch (error) {
      ...
      if (cancelResult.refundAmount !== "0") {
        await prisma.transaction.create({
          data: {
            ...
            amount: cancelResult.refundAmount,
            status: TransactionStatus.CONFIRMED,
            fromAddress: process.env.ESCROW_CONTRACT_ADDRESS || "0xLiveLedgerContract",
            toAddress: stream.payer.walletAddress,
            streamId: stream.id,
            txHash: cancelResult.transactionHash,
          },
  ...
  async getStream(streamId: string, userId: string): Promise<any> {
    ...
    const enhancedStream = await this.balanceService.enhanceStreamWithBalance(stream);

    // Add withdrawal limit info for recipients
    if (stream.recipientId === userId) {
      const withdrawalInfo = this.withdrawalService.getWithdrawalLimitInfo(stream.recipient.walletAddress);
      return {
        ...enhancedStream,
  ...
  async getUserStreams(userId: string): Promise<any[]> {
    ...
    const enhancedStreams = await Promise.all(
      streams.map(async (stream) => {
        const enhanced = await this.balanceService.enhanceStreamWithBalance(stream);
  ...
  async getWithdrawalLimitInfo(userAddress: string) {
    return this.withdrawalService.getWithdrawalLimitInfo(userAddress);
  }
}